...
static void rx_timeout(struct k_timer *timer)
{
	...
	if (len > 0) {
		if (clipped ||
			data->async->rx_timeout_left < data->async->rx_timeout_slab) {
...
static void endrx_isr(struct device *dev)
{
	 ...
	 * buffer
	 */
	const int this_amount = nrf_uarte_rx_amount_get(uarte);

	/* it is possible for rx_offset to be more than the length of the
	 * EasyDMA transfer, as the rx_offset comes from counts of the RXRDY
	 * event, which occurs before data is transferred via DMA
	 * So in these cases we clip rx_len
	 */
	bool has_excess = false;
	int rx_len = this_amount - data->async->rx_offset;
	if (rx_len < 0) {
		rx_len = 0;
		has_excess = true;
	}
